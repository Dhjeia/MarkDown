...
弱平稳（宽平稳）：满足以下条件：
	1.	均值是常数，不随时间变化。
	2.	方差是常数，不随时间变化。
	3.	自协方差只取决于时间滞后 h，而与具体时间 t 无关。
## 样本自相关图（ACF 图）
定义：
ACF 图展示了不同滞后 h 的样本自相关系数 r(h)。

特征解读：
	如果自相关系数快速衰减到零，序列可能是平稳的。

## 纯随机序列（白噪声）
白噪声序列的自相关系数  \rho_k  在滞后  k                                          
白噪声序列一定是平稳序列

## 
	•	自回归系数：它告诉我们当前的值和过去的值有多强的关系。就像你看昨天的气温，来预测今天的气温。这个关系是通过一个模型来计算的。
	•	自相关系数：它告诉我们不同时间点之间的整体关系。比如你想知道今天的气温和前天、后天的气温有多相关，不仅仅是昨天和今天的关系。它是通过计算整个数据集的相关性来得到的。
...
| 字母  | 含义                                                         |
|-------|--------------------------------------------------------------|
| \( t \)   | 时间点（通常为整数，如 \( t = 1, 2, 3, \dots \)）                  |
| \( X_t \) | 时间序列值，表示时间 \( t \) 时的观测值或变量值                       |
| \( Y_t \) | 时间序列的另一个变量值（例如，双变量时间序列中使用）                 |
| \( \epsilon_t \) | 随机扰动或噪声项，通常假设为独立同分布（i.i.d.）白噪声 \( N(0, \sigma^2) \) |
| \( \phi \)  | 自回归（AR）模型的系数，表示当前值对前值的影响权重                |
| \( \theta \) | 移动平均（MA）模型的系数，表示当前值对噪声项的影响权重            |
| \( Z_t \) | 白噪声序列，常用于表示无序且独立的随机过程                          |
...
| \( AR(p) \) | \( p \)-阶自回归模型                                           |
| \( MA(q) \) | \( q \)-阶移动平均模型                                         |
| \( ARMA(p, q) \) | \( p \)-阶自回归和 \( q \)-阶移动平均模型的组合             |
| \( ARIMA(p, d, q) \) | 差分自回归移动平均模型，其中 \( d \) 表示差分次数           |
| \( SARIMA(p, d, q)(P, D, Q)_s \) | 季节性 ARIMA 模型，\( s \) 表示季节周期        |
...
| \( Q \)      | Ljung-Box 检验统计量，用于检验序列是否为白噪声               |
| \( p \)-value | 检验的显著性概率值                                         |
...
| 字母         | 含义                                                         |
|--------------|--------------------------------------------------------------|
| \( \Delta \) | 差分算子，例如 \( \Delta X_t = X_t - X_{t-1} \)              |
| \( B \)      | 延迟算子，\( B X_t = X_{t-k} \)                              |
| \( \Delta^d \) | 差分次数为 \( d \) 的多项式                                  |
| \( \Delta^d Y_t \) | 差分次数为 \( d \) 的时间序列                                |
